...
Изучение работы STP проводилось в рамках базового курса. Ссылка на работу: (https://github.com/AlexeyKislitsyn/AlexeyKislitsyn/tree/main/Basic/lab07). Здесь же я хочу рассмотреть работу мультивендорного протокола MST.

Протокол MST был разработан для устранения недостатка или избытка экземпляров STP. В результате инженер может настроить точно такое количество экземпляров STP, которое имеет смысл для данной сети.
...
На Cisco есть ограничение в количестве инстанций STP или RSTP на одном коммутаторе в зависимости от модели. То есть добавив 128 вланов на каком-то коммутаторе, мы сталкнемся с таким ограничением. Все эти проблемы успешно решает мультивендорный протокол MSTP.

Постараемся разобраться с его работой.
...
|-------------|-------------|----------------|----------------|
| S1          | VLAN 100    | 192.168.100.1  | 255.255.255.0  | 
| S2          | VLAN 100    | 192.168.100.2  | 255.255.255.0  |  
| S3          | VLAN 100    | 192.168.100.3  | 255.255.255.0  |
...
S2, S3 - Аналогичная настройка, исключением является настройка ip адреса vlan 100 согласно таблицы адресации.
...
Но может сложиться ситуация, что коммутатор S1 выйдет из строя или отвалятся линки от S3 к S1, тогда получается что у нас опять утилизирован один линк e 0/2 в сторону S2:
